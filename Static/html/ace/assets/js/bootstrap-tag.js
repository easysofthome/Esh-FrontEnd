...
!function ( $ ) {
  ...
  , show: function () {
      ...
      if (that.options.arrayList.length) {
        $.each(that.arrayList, function () {
          that.createBadge(this.value)
        })
      }
  ...
  // //声明对象
  // , ObjStory: function(array){
  //     for(term in array){
  //       this.term = array[term];
  //     }
  // }
  , add: function ( array ) {
      var that = this
      ...
      if ( !that.options.allowDuplicates ) {
        var index = that.inValues(array.value)
        if ( index != -1 ) {
      ...
      this.createBadge(array.value)

      this.element.val(this.values.join(', '))
      this.element.trigger('added', [array.value])
    }
  ...
  , source: []
  , arrayList: ''
  // , arrayList: [{'id': 1,'name': 'john'},{'id': 2,'name': 'tom'}]
  }
