  ...
  function scrollToTarget(Snum,callback) {
    ...
    if (options.steps[_index].autoScroll || options.steps[_index].autoScroll == undefined) {
      if (options.steps[_index].popup.position != 'modal') {
        ...
        highlightWapper = $(options.steps[_index].wrapper),
         // targetOffsetTop = $jpwTooltip.offset().top,
        targetOffsetTop = highlightWapper.offset().top,
        targetHeight =  highlightWapper.height() ||  highlightWapper.innerHeight(),
        overlayTop = $('#overlayTop').height();
        var appendScrollNum = 0;
        if(Snum===0){
            appendScrollNum= options.steps[_index].appendScrollNum;
        }else{
            appendScrollNum=0;
        }

        var scrollTopNum = targetOffsetTop + (targetHeight / 2) - (windowHeight / 2);
        if(options.steps[_index].isTopFix){
          scrollTopNum = 0;
        }

        //不需要滚动，则直接执行回调
        if(scrollTopNum === 0 && $(document).scrollTop() === 0){
        ...
        //需要滚动,执行完动画后执行回调
        $('html,body').animate({
          scrollTop: scrollTopNum + parseInt(appendScrollNum)

        }, options.steps[_index].scrollSpeed,function(){
         if(callback != undefined && typeof(callback) == 'function'){
            callback();
         }
       });
